...
3. **Work** — the standards below apply throughout
...
**Where it goes:** `roles/{receiver}/HANDOFF-{short-description}.md`

Example: Sisko asks Dax to architect the relay service → `roles/dax/HANDOFF-RELAY-SERVICE.md`
...
- The artifact lives in the **receiver's** folder, not the sender's. The receiver's folder is their inbox.
...
Naming: `roles/{original-sender}/RESPONSE-{short-description}-FROM-{responder}.md`

Example: Dax reviews Sisko's relay service handoff and returns an architecture recommendation → `roles/sisko/RESPONSE-BET2-ARCHITECTURE-FROM-DAX.md`
